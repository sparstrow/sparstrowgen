...
| Browser-approved machine login without copying a shared daemon secret | **Built 2026-09-13.** The browser mints an opaque ten-minute request; the local daemon spends it exactly once, receives a distinct 256-bit credential, writes it in its per-user configuration directory, and waits for browser approval before it can open the daemon websocket. Only hashes are stored. The legacy `DAEMON_TOKEN` route remains additive during migration. |
...
| Disconnecting one computer | **Built 2026-09-13.** Revocation is per-machine, closes that machine's live socket, and rejects future websocket dials with its credential. Other paired machines remain connected. |
