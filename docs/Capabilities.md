...
| Browser-approved machine login without copying a shared daemon secret | **Not built here; feasible, but Multica is not current proof of the final credential flow.** Its current `multica login` opens the browser, receives a short-lived browser credential through a loopback callback, then mints and stores a user PAT. Multica's earlier `daemon_pairing_session` table was removed in migration 029, and its own daemon-auth test says normal daemon traffic still uses user PATs because `mdt_` minting is not wired. Sparstrowgen can reuse the browser-approved shape, but must define and prove its own one-time claim and per-computer credential rather than copying the retired table or treating Multica as proof. |
| The browser recognising that the local component is installed or running | **Not built; platform-supported approach chosen, clean-machine proof still required.** The daemon dials out only, so the browser cannot ask it directly. Probing `localhost` from the hosted site is rejected: it breaks the no-inbound rule and Chrome now gates public-site requests to the local network behind a permission prompt. The installer instead registers a per-user `sparstrowgen://` link handler, which Windows supports for packaged and unpackaged desktop apps ([Microsoft URI activation guidance](https://learn.microsoft.com/en-us/windows/apps/develop/launch/handle-uri-activation)). Setup opens that link from an explicit user action with an opaque one-time pairing request; the local component validates the scheme, action and request before claiming it by dialling out to the server. The URI itself proves no caller identity and every parameter is untrusted. **Absence cannot be proved** — an unanswered link looks the same as a slow start or a blocked prompt — so a design shows "not found yet" with both retry and install, never "not installed". This session's disposable registry probe was blocked by the command guard before it changed anything; verify the installed handler on a clean Windows machine before the US2 backend. |
