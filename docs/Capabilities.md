...
| Invitation-only multi-user registration without a server-log setup code | **Not built.** The current database and signup path deliberately permit exactly one account and require the startup code. Replacing that boundary must preserve the existing account. Registration is limited to addresses the owner has allowed; for this release the allowed list may be hosting configuration, because an in-product invitation surface is out of scope. |
| An uninvited sign-up recorded as an access request, and the owner told | **Not built.** Needs a request record per address (repeat attempts collapse into one and are throttled like sign-in) and the same email path as confirmation, to notify the owner. Until the administration place exists (L-18), approving means allowing the address the same way as an invitation. |
| One account cannot see another's conversations or reach another's computer | **Not built — and a precondition for registration, not a follow-up.** Conversations carry no owner and the hub holds one daemon connection shared by every browser. See [`KnownGaps.md`](KnownGaps.md) G-27. |
| Proving a new person's email address, and resetting a forgotten password | **Not built.** No email-delivery path is configured in this repository. Recommended: a transactional email provider called over HTTPS rather than self-run SMTP, because deliverability (SPF/DKIM, reputation) is the hard part and it is not ours to solve. **No provider is selected or verified yet**; selecting one needs the owner to add DNS records on the sending domain (a runbook step). Designs may show the outcome; the backend stage waits for a verified send. **Found 2026-09-12:** the owner has a Hostinger mailbox on the product domain, `agent@sparstrow.com`, reachable through the Hostinger Email API (`GET /api/v1/me` answered; send, list and read operations exist). It is a real inbox agents may use to receive and click verification and reset emails end to end. Whether the *server* should send through Hostinger's SMTP for the same domain, instead of a transactional provider, is still open; at invitation-only volume it avoids a new vendor. Nothing has been sent from it yet. |
