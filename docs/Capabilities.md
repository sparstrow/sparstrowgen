...
| The browser recognising that the local component is installed or running | **Built locally 2026-09-13; clean-machine release proof remains.** The Windows launcher registers a per-user `sparstrowgen://` handler, validates the opaque `pair` request, claims it by dialling out to the server, and starts the paired daemon after the browser approves. The setup surface opens that URI from an explicit user action; after eight seconds without a claim it honestly offers retry and installer help rather than asserting that the component is absent. The local registry registration and package build are verified. A signed, published installer and a clean-Windows browser-to-handler proof remain required before this can be called a production installation path. |
