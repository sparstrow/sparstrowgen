...
func (s *Store) DeleteEveryUser(ctx context.Context) (int64, error) {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	q := s.q.WithTx(tx)
	if err := q.DeleteEveryConversationFileContent(ctx); err != nil {
		return 0, err
	}
	if err := q.DeleteEveryConversationFile(ctx); err != nil {
		return 0, err
	}
	if err := q.DeleteEveryExchangeContext(ctx); err != nil {
		return 0, err
	}
	if err := q.DeleteEveryContextDocument(ctx); err != nil {
		return 0, err
	}
